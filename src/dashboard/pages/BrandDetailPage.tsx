        ...
        if (active) {
          setError(
            err instanceof ApiError
              ? err.message
              : 'Marka yüklənərkən xəta baş verdi.',
          )
        }
      } finally {
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      setError(
        err instanceof ApiError
          ? err.message
          : 'Marka silinərkən xəta baş verdi.',
      )
      setDeleteOpen(false)
    } finally {
      setDeleting(false)
  ...
  return (
    <div className="dash-page">
      <header className="dash-page__header dash-page__header--row">
        <div>
          <h1 className="dash-page__title">
            {item ? nameOf(item, 'az') : 'Marka'}
          </h1>
        </div>
        <div className="dash-page__actions">
          <Link to="/dashboard/brands" className="dash-btn dash-btn--ghost">
            Siyahıya qayıt
          </Link>
          {item && (
            <>
      ...
      <ConfirmModal
        ...
        message={item ? `“${nameOf(item, 'az')}” markası silinəcək.` : ''}
        confirmLabel="Sil"
        cancelLabel="Ləğv et"
        confirming={deleting}
